...
def main(filter, user_t_exp, pixel_scale, dust=True, noise=True, catalogue_path='',center=None,fov_name=-99):
    ...
    np.savetxt(output_messages_path+'GALEX_AaronsCat_wide{}_'.format(simulation_num) +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') +'FFTerrors.txt', FFTerror)
    np.savetxt(output_messages_path+'GALEX_AaronsCat_wide{}_'.format(simulation_num) +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') + 'MemErrors.txt', MemError)
    
    for t_exp in user_t_exp:
        #  Add noise to field image and save
        file_name1 = os.path.join(output_images_path+'GALEX_AaronsCat_wide{}_'.format(simulation_num)  +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) +'s_pixscale'+str(pixel_scale).replace('.','p') + '.fits')
        if noise == True:
