...
def main(filter, user_t_exp, pixel_scale, dust=True, noise=True, catalogue_path=''):
    ...
    np.savetxt(output_messages_path+'LSST_RomanStarCat_2019PSFs_' + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') +'FFTerrors.txt', FFTerror)
    np.savetxt(output_messages_path+'LSST_RomanStarCat_2019PSFs_' + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') + 'MemErrors.txt', MemError)
    
    for t_exp in user_t_exp:
        #  Add noise to field image and save
        file_name1 = os.path.join(output_images_path+'LSST_RomanStarCat_2019PSFs_' + filter + '_' + str(t_exp) +'s_pixscale'+str(pixel_scale).replace('.','p') + '.fits')
        if noise == True:
