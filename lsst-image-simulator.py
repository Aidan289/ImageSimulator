...
def main(filter, user_t_exp, pixel_scale, dust=True, noise=True, catalogue_path='',center=None,fov_name=-99):
    ...
    relevant_columns=['ra', 'dec', 'LSST_g', 'LSST_g_bulge', 'LSST_g_dust', 'LSST_u_split_bb', 'LSST_u_split_bb_bulge', 'LSST_u_split_bb_dust', 'LSST_u', 'LSST_u_bulge', 'LSST_u_dust', 'LSST_uv_split_bb', 'LSST_uv_split_bb_bulge', 'LSST_uv_split_bb_dust', 'LSST_uv', 'LSST_uv_bulge', 'LSST_uv_dust', 'r_disk', 'rbulge',
                                     'cosi', 'redshift']
    ...
    #  Create field image based on LSST specifications
    ...
    for folder in os.listdir(directory):
        ...
        # intrinsic mags
        disk_mag = np.array(data['LSST_' + filter])
        bulge_mag = np.array(data['LSST_' + filter + '_bulge'])
        # dust mag (disk+bulge combined)
        dust_mag = np.array(data['LSST_' + filter + '_dust'])
        ...
        for i in range(0, tot):
            ...
            for t_exp in user_t_exp:  # exposure time in seconds
                ...
                #  Convolve with PSF
                final = galsim.Convolve([gal, lsst_psf])
    ...
    np.savetxt(output_messages_path+dt+'_LSST_AaronsCat_wide{}_2019PSFs_'.format(simulation_num) +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') +'FFTerrors.txt', FFTerror)
    np.savetxt(output_messages_path+dt+'_LSST_AaronsCat_wide{}_2019PSFs_'.format(simulation_num) +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) + 's_pixscale'+str(pixel_scale).replace('.','p') + 'MemErrors.txt', MemError)
    
    for t_exp in user_t_exp:
        #  Add noise to field image and save
        file_name1 = os.path.join(output_images_path+dt+'_LSST_AaronsCat_wide{}_2019PSFs_'.format(simulation_num)  +'_FOV{}_'.format(fov_name) + filter + '_' + str(t_exp) +'s_pixscale'+str(pixel_scale).replace('.','p') + '.fits')
        if noise == True:
            ...
            ccd_noise = galsim.CCDNoise(rng, sky_level=sky_level, gain=g,
                                        read_noise=9.)  # includes read noise, sky noise and shot noise, sky_level in ADU/pixel
            image[t_exp].addNoise(ccd_noise)
            print("Added CCD noise (including read noise, sky noise and shot noise), "+str(ccd_noise))
...
if __name__=='__main__':
    ...
    exp_times = [30,300]#39000,3000,18000,2000]  # enter any exposure time in seconds (s)
    ...
    for fov_id in [0]: #number in 0-5, chooses which FOV from the 6 that will fit in the full catalogue area
        print('')
